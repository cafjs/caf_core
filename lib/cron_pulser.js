...
exports.newInstance = function(context, spec, secrets, cb) {
    ...
    that.start(function() {
                   $.log && $.log.debug('Cron ' + that.getName() +
                                        ' waking up');
                   var pulseF = function(tuple, cb0) {
                       if (tuple.value.isShutdown) {
                           $.lookup.remove(tuple.key);
                           cb0(null);
                       } else {
                           tuple.value.pulse(cb0);
                       }
                   };
                   ...
                   $.lookup.forEachAsync(pulseF, cb0);
               });

    $.log && $.log.debug('New pulser cron job');
    cb(null, that);
